...
- KC Browser research uses server-side Exa source discovery when `EXA_API_KEY` is configured. Exa is used only to find sources; selected pages are retrieved through the bounded `web.fetch/read` path as untrusted data. Brave remains available through the explicit `KC_AI_WEB_SEARCH_PROVIDER=brave` configuration. Without a configured provider, `web.search` remains blocked and KC AI does not fabricate results.
