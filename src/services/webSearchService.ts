...
export function getWebSearchProvider(): SearchProvider { return new BraveSearchProvider(); }

export function getWebSearchConfiguration(): { configured: boolean; provider?: string; reason?: string } {
  const provider = providerName();
  if (!provider) return { configured: false, reason: 'KC_AI_WEB_SEARCH_PROVIDER is not configured' };
  if (provider !== 'brave') return { configured: false, provider, reason: `Unsupported web search provider: ${provider}` };
